...
from geral.functions import dict_to_rec_trac_log, log_version_by_table
...
def post_save_tracking(sender, instance, **kwargs):
    ...
    if hasattr(instance, '__original_record_values'):
        ...
        if original['id'] == getattr(instance, 'id'):
            ...
            rt.iud = 'u'
            rt.log_version = log_version_by_table(rt.table)
            rt.log = dict_to_rec_trac_log(altered, rt.log_version)
            rt.save()
