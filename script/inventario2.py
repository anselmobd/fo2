...
class Inventario:
    ...
    def mes(self, value):
        if isinstance(value, int):
            self._mes = f"{value:02}"
        else:
            self._mes = value

    def abre_csv(self):
        try:
            self._csvfile = open(self.csv, newline='')
            self._csvreader = csv.DictReader(
                self._csvfile, delimiter=';', quotechar='"')
        except Exception as e:
            print(f"Não consegui abrir para leitura {self.csv}")
            sys.exit(92)
    ...
    def verify(self):
        if self.origem == 'p':
            if self.csv:
                ...
                print(
                    "Inventario - Erro de inicialização: "
                    "origem == 'p', exige parâmetro csv"
                )
                sys.exit(91)
    ...
    def get_quant_systextil_sql(self, nivel, ref):
        ...
        sql = f"""
            ...
            LEFT JOIN estq_310 e
             ...
             and e.SEQUENCIA_INSERCAO = s.SEQ_BUSCA
            JOIN basi_030 r
...
def parse_args():
    ...
    parser.add_argument(
        "origem",
        choices=['s', 'f', 'p'],
        help='Origem dos dados (Systextil, Fo2-intranet ou Planilha-CSV)')
    parser.add_argument(
        "-p", "--prolong",
    ...
    parser.add_argument(
        "-r", "--ref",
        type=str,
        help='Força apenas essa referência')
